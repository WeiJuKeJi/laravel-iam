...
- **统一菜单来源**：所有前端路由节点都落库至 `menus` 表，保持可视化维护与权限联动。字段要点如下：
  ...
  - `guard`：JSON，支持两种形态：
    - 数组：角色白名单，例如 `['Admin']`；
    - 对象：`{ "role": ["Editor"], "mode": "except" }` 表示黑名单。
- **多对多绑定**：
  - `menu_role`、`menu_permission` 用于将菜单与 Spatie Permission 的角色/权限建立绑定，可满足更细粒度控制需求。
...
2. **权限过滤**：`MenuService::getMenuTreeForUser()` 会收集当前用户的角色与权限，并逐节点调用 `Menu::isVisibleFor()`：
   - 若节点绑定了角色但当前用户不在绑定列表内，则直接剔除；
   - 若节点存在 `guard` 数组/对象，也会结合 `include/except` 规则再次筛选；
   - 若父节点自身无权限但存在可见子节点，会保留父节点以保证前端路由结构完整。
3. **版本号生成**：根据菜单集合最近更新时间和节点总数生成 MD5，形成 `version`；接口响应头同时返回 `X-Menu-Version`，便于前端缓存对比。
4. **缓存策略**：默认缓存 30 分钟；若缓存驱动支持标签（如 Redis），将使用 `menus` 标签，否则回退到全局缓存键。所有写操作及种子脚本执行后都会主动刷新。
...
| 接口 | 方法 | 权限 | 用途 |
| --- | --- | --- | --- |
| `/api/v1/iam/menus` | `GET` | `iam.menus.view` | 查看完整菜单树（含角色/权限绑定）。 |
...
- **创建/更新请求体关键字段**：
  ...
  - `role_ids` / `permission_ids`：数组形式的绑定 ID。
...
2. **菜单级**：
   - `menu_role`/`menu_permission` 的绑定优先级最高，未命中则直接隐藏；
   - `guard` 字段用于对角色做轻量配置（典型场景：管理员专属或排除部分角色）。
3. **与 Spatie Permission 的配合**：由于接口返回中会列出 `roles`/`permissions` 字段，前端可在菜单管理页面直观展示绑定情况。
